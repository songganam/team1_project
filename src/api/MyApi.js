...
export const getMyReview = async ({ param, successFn, failFn, errorFn }) => {
  try {
    // const res = await axios.get(`${host}/user/review`, { params: param });
    const res = await axios.get("/json/myreview.json", { params: param });
    const status = res.status.toString();
    if (status.charAt(0) === "2") {
...
export const getMyList = async ({ param, successFn, failFn, errorFn }) => {
  try {
    // const res = await axios.get(`${host}/user/bookmark`, { params: param });
    const res = await axios.get("/json/mylist.json", { params: param });
    const status = res.status.toString();
    if (status.charAt(0) === "2") {
...
export const getUserInfo = async ({ param, successFn, failFn, errorFn }) => {
  try {
    ...
    const res = await axios.get("/json/user.json", { params: param });
    const status = res.status.toString();
    if (status.charAt(0) === "2") {
