...
const MeatReviewPage = () => {
  ...
  //   const handleStarClick = (star: MouseEvent<HTMLImageElement> | number) => {
  ...
  const handleStarClick = (star: MouseEvent<HTMLImageElement> | number) => {
    setReviewData(prevData => ({
      ...
      dto: {
        ...prevData.dto,
        star: typeof star === "number" ? star : prevData.dto.star,
      },
    }));
  };
  ...
  // 이미지 미리보기에서 삭제 함수
  const deleteImage = (
    indexToDelete: MouseEvent<HTMLInputElement> | number,
  ) => {
  ...
  const addMutation = useMutation({
    mutationFn: (reviewData: FormData) => postReviewTS({ reviewData }),
    onSuccess: result => {
      setFetching(false);
  ...
  // 파일 업로드 실행
  const handleClick = async (reviewData: ReviewForm) => {
    ...
    const dto = new Blob(
      [
        JSON.stringify({
          ...
          ishop: ishop,
          checkShop: checkShop,
          star: reviewData.dto.star,
          review: reviewData.dto.review,
        }),
      ],
      // JSON 형식으로 설정
  ...
  // 확인 버튼 클릭 시
  const handleConfirm = (reviewData: ReviewForm) => {
    // 글 등록 로직 실행
            ...
            <ReviewFormWrap>
              ...
              <ReviewRating>
                {[1, 2, 3, 4, 5].map(star => (
                  <ReviewRatingStar
                    key={star}
                    src={star <= reviewData.dto.star ? countStar : noCountStar}
                    alt=""
